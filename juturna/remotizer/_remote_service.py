"""
gRPC Server endpoint that receives and deserializes MessageEnvelope
...
def deserialize_audio_payload(payload: AudioPayload) -> dict[str, Any]:
...
def deserialize_image_payload(payload: ImagePayload) -> dict[str, Any]:
...
def deserialize_video_payload(payload: VideoPayload) -> dict[str, Any]:
...
def deserialize_bytes_payload(payload: BytesPayload) -> dict[str, Any]:
...
def deserialize_object_payload(payload: ObjectPayload) -> dict[str, Any]:
...
def deserialize_batch(payload: Batch) -> dict[str, Any]:
...
def deserialize_envelope(envelope: MessageEnvelope) -> dict[str, Any]:
    """Deserialize complete MessageEnvelope to Python dict"""
    return {
        'id': envelope.id,
...
class MessagingServiceImpl(messaging_service_pb2_grpc.MessagingServiceServicer):
    ...
    def _register_default_handlers(self):
        """Register default handlers for each payload type"""

        def handle_audio(envelope_dict):
            logger.info(f'Processing audio from {envelope_dict["sender"]}')
            audio_data = envelope_dict['message']['payload']['audio']
            logger.info(
                f'Audio shape: {audio_data.shape}, '
                f'sampling rate: {envelope_dict["message"]["payload"]["sampling_rate"]}'
            )
            # Your audio processing logic here
            return {'status': 'audio processed'}

        def handle_image(envelope_dict):
            logger.info(f'Processing image from {envelope_dict["sender"]}')
            image_data = envelope_dict['message']['payload']['image']
            logger.info(f'Image shape: {image_data.shape}')
            # Your image processing logic here
            return {'status': 'image processed'}

        def handle_video(envelope_dict):
            logger.info(f'Processing video from {envelope_dict["sender"]}')
            num_frames = envelope_dict['message']['payload']['num_frames']
            logger.info(f'Video frames: {num_frames}')
            # Your video processing logic here
            return {'status': 'video processed'}

        def handle_bytes(envelope_dict):
            logger.info(f'Processing bytes from {envelope_dict["sender"]}')
            content_size = len(envelope_dict['message']['payload']['content'])
            logger.info(f'Content size: {content_size} bytes')
            # Your bytes processing logic here
            return {'status': 'bytes processed'}

        def handle_object(envelope_dict):
            logger.info(f'Processing object from {envelope_dict["sender"]}')
            obj_data = envelope_dict['message']['payload']
            logger.info(f'Object keys: {obj_data.keys()}')
            # Your object processing logic here
            return {'status': 'object processed'}

        def handle_batch(envelope_dict):
            logger.info(f'Processing batch from {envelope_dict["sender"]}')
            num_messages = envelope_dict['message']['payload']['num_messages']
            logger.info(f'Batch contains {num_messages} messages')
            # Your batch processing logic here
            return {'status': 'batch processed'}

        # Register all handlers
        self.handler.register('AudioPayload', handle_audio)
        self.handler.register('ImagePayload', handle_image)
        self.handler.register('VideoPayload', handle_video)
        self.handler.register('BytesPayload', handle_bytes)
        self.handler.register('ObjectPayload', handle_object)
        self.handler.register('Batch', handle_batch)

    def SendMessage(self, request: MessageEnvelope, context):
    ...
    def SendAndReceive(self, request: MessageEnvelope, context):
